...
#pragma config(Motor,  port6,           waterOrgServo, tmotorServoStandard, openLoop)
#pragma config(Motor,  port7,           mannyMagServoOne, tmotorServoStandard, openLoop)
...
bool waterOrgCheck = false;
bool waterOrgOn = false;
bool orgSwap = false;
bool orgCheck = false;
/*
...
void orgWater(){
	if((vexRT[Btn7D]==true)&&(waterOrgCheck==false)){
		waterOrgOn = !waterOrgOn;
		waterOrgCheck=true;
	}
	if((vexRT[Btn7D]==false)&&(waterOrgCheck==true)){
		waterOrgCheck = false;
	}
	if(waterOrgOn==true){
		...
		if(orgSwap == true){
			motor(waterOrgServo) = 90;
			}else if(orgSwap == false){
			motor(waterOrgServo) = 0;
		}
	}
}
