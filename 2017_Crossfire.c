...
bool speedCheck = false; //toggle boolean the speed scaler value described below
int speedScaler = 1;//This is what allows are drivers to switch between back and forth between 100% speed and 50% speed for fine tune controll or speed. Effects only the drive systems
...
void arcadeDrive(){	//Function for arcade drive
	...
	if( abs( vexRT[Ch1] ) < tolerance && abs ( vexRT[Ch2] ) < tolerance){
		...
		motor[motorLeft] = ((vexRT[Ch2]+vexRT[Ch1]))/(2*speedScaler);
		motor[motorRight] = ((vexRT[Ch2]-vexRT[Ch1]))/(-2*speedScaler);
		...
		motor[motorLeft] = ((vexRT[Ch2]+vexRT[Ch1]))/(2*speedScaler);
		motor[motorRight] = ((vexRT[Ch2]-vexRT[Ch1]))/(-2*speedScaler);//For our speed scaler we either divide, or mulitply a divisor. Either works
	}
}
...
void tankMovement(){
	...
	if(abs(vexRT(Ch2))<=10 && abs(vexRT(Ch3))<=10){
		...
		motor[motorRight] = vexRT[Ch2] * -1/speedScaler;//We can normaly just divide the channel by speed scalar for the gearshift, However dividing -1 by the scalar has the same effect
	}
}
void toggleSpeed(){
	//Toggle switch. Btn7D is pressed it switches between /2 and /1 This can half the speed for finer controll.

	if((vexRT[Btn7D]==1)&&(speedCheck==false)){
		if(speedScaler==1){
			speedScaler=2;//Since its a division 2 symbolises 1/2
		}else{
			speedScaler=1;//Since its a division 1 is maximum speed
		}
		speedCheck = true;
	}
	if((vexRT[Btn7D]==false)&&(speedCheck==true)){
		speedCheck = false;
	}
}
void toggleArcade(){
	//Toggle switch. Btn7D is pressed, operating value inverts. Changes between arcade and tank drive.
...
void drive(){
	...
	toggleArcade();
	toggleSpeed();//This is to switch speed from 100% to 50% on a toggle
...
void rescue(){
	...
	mannyMagnet();//Run the function that controlls the servo's that help manny
}

//Task main is the beginning of the program
task main()
{
	//Infinite loop causes infinite execution(So it will run the entirety of the game)
	while(1==1){
		drive();
		shoot();
		rescue();
		//Each function contains the entirety of said function as a way to abstract out the code
	}
}
